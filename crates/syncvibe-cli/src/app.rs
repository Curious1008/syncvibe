...
pub async fn run() -> Result<()> {
    ...
    loop {
        ...
        if terminal
            .draw(|frame| crate::render::draw_ui(frame, &mut state))
            .is_err()
        {
            break;
        }
        ...
        tokio::select! {
            ...
            maybe_event = key_rx.recv() => {
                match maybe_event {
                    Some(Event::Key(key)) => {
                        if let Err(e) = crate::events::key::handle_key_event(&mut state, key) {
                            state.toast_err(&format!("Error: {}", e));
                        }
                    }
                    Some(Event::Mouse(mouse)) => {
                        crate::events::mouse::handle_mouse_event(&mut state, mouse);
                    }
                    _ => {}
                }
            }
        ...
        if state.want_new_project {
            ...
            let switched = crate::flows::project::handle_new_project();
            if !switched {
        ...
        if state.want_join_project {
            ...
            let switched = crate::flows::project::handle_join_project();
            if !switched {
                terminal = tui::setup()?;
                key_rx = spawn_key_bridge();
                continue;
